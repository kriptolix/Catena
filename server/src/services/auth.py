...
def create_initial_admin(db: sqlite3.Connection):
    ...
    cursor = db.execute("SELECT COUNT(*) as count FROM user")
    ...
    if count == 0:
        ...
        db.execute(
            """
            INSERT INTO user (username, fullname, hashed_password, is_admin, active)
            VALUES (?, ?, ?, ?, ?)
            """,
            ("admin", "administrator", admin_password, 1, 1)
        )
        db.commit()
